...
class WeightDependentPostPre(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        G_neg= -3.5*(0.137 + np.exp(-d/1230))
        G_pos= 0.046*(23.2 + d*np.exp(-d/790))
        c_neg= 1.0*(-0.53 + np.exp(-d/810))
        c_pos= -1.7*(0.01 + np.exp(-d/130))
        t= 0.14*(-670 + d*np.exp(-d/900))
        ...
        outer_product = self.reduction(torch.bmm(source_s, (target_x/self.source.tc_trace)*113), dim=0)
        update += G_neg * outer_product * (self.connection.w - self.wmin) + c_neg*self.reduction(source_s,dim=0)

        # Post-synaptic update.
        
        outer_product = self.reduction(torch.bmm(  ((source_x/self.source.tc_trace)*t)  , target_s), dim=0)
        update += G_pos * outer_product * (self.wmax - self.connection.w) + c_pos*self.reduction(target_s,dim=0)
