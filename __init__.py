...
class WeightDependentPostPre(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if self.connection.type_of_connection == 0:    
            self.connection.w += update
        if self.connection.type_of_connection == 1:    
            self.connection.w += torch.diagflat(torch.diagonal(update, 0))
        if self.connection.type_of_connection == 2:    
            self.connection.w += (update-torch.diagflat(torch.diagonal(update, 0)))
